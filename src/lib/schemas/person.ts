...
// schema with an optional `jobsite_name` (the raw value the operator
// mapped from a CSV Jobsite column). The server resolves it to an
// active jobsite ID at insert time. Max length matches the jobsite name
...
export const importPersonRowSchema = personInputSchema.extend({
  jobsite_name: z
    .string()
    .transform((s) => s.trim())
    .pipe(z.string().max(200, "Jobsite name is too long"))
    .optional(),
});
